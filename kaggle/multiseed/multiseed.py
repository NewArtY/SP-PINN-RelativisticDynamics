...
configs = []
# E4b - MULTI-SEED error bars on the scaling law (width 256, full recipe, 8k ep).
SEEDS = [0] if SMOKE else [0, 1, 2]
NS = [8000] if SMOKE else [8000, 32000, 128000]
EP = 60 if SMOKE else 8000
for N in NS:
    for sd in SEEDS:
        configs.append(mk(f"ms_w256_N{N}_s{sd}", width=256, N=N, seed=sd, epochs=EP))
...
# ---- multi-seed aggregation: eps_theta mean +- std vs N ----
import collections
agg = collections.defaultdict(list); aggt = collections.defaultdict(list)
for r in allres:
    agg[r["N"]].append(r["eps_theta"]); aggt[r["N"]].append(r["traj_err"])
Ns = sorted(agg); summary = {}
means = [float(np.mean(agg[N])) for N in Ns]; stds = [float(np.std(agg[N])) for N in Ns]
for N in Ns:
    summary[str(N)] = {"eps_mean": float(np.mean(agg[N])), "eps_std": float(np.std(agg[N])),
                       "traj_mean": float(np.mean(aggt[N])), "traj_std": float(np.std(aggt[N])), "n_seeds": len(agg[N])}
    print("N=%6d  eps=%.3e +- %.1e  traj=%.3f +- %.3f  (n=%d)"
          % (N, summary[str(N)]["eps_mean"], summary[str(N)]["eps_std"],
             summary[str(N)]["traj_mean"], summary[str(N)]["traj_std"], len(agg[N])), flush=True)
json.dump({"results": allres, "summary": summary, "elapsed_s": time.time()-t_all}, open(OUT+"/results.json", "w"), indent=2)
try:
    plt.figure(figsize=(5.2, 3.8))
    plt.errorbar(Ns, means, yerr=stds, fmt="o-", capsize=4, label="width 256 (3 seeds)")
    if len(Ns) >= 2:
        a, b = np.polyfit(np.log(Ns), np.log(means), 1); print("scaling fit: eps ~ N^%.3f" % a, flush=True)
        xx = np.array([min(Ns), max(Ns)], float); plt.plot(xx, np.exp(b)*xx**a, "k:", alpha=.6, label=r"fit $\propto N^{%.2f}$" % a)
    plt.axhline(1e-4, ls="--", color="r", alpha=.4, label=r"$10^{-4}$ target")
    plt.xscale("log"); plt.yscale("log")
    plt.xlabel("collocation points N"); plt.ylabel(r"$\varepsilon_\theta$ (in-tube RMS)")
    plt.title("Surrogate accuracy scaling (multi-seed)"); plt.legend(fontsize=8); plt.grid(True, which="both", alpha=.3)
    plt.tight_layout(); plt.savefig(OUT+"/scaling_multiseed.png", dpi=140)
